...
function loadPlaces(map) {
  ...
  _axios2.default.get('/api/stores/near?lat=' + lat + '&lng=' + lng).then(function (res) {
    ...
    // when someone clicks on a marker, show the details of that plce
    ...
    // then zoom map to fit all of the markers perfectly
...
function typeAhead(search) {
  ...
  searchInput.on('input', function () {
    ...
    _axios2.default.get('/api/search?q=' + this.value).then(function (res) {
      ...
      searchResults.innerHTML = _dompurify2.default.sanitize('<div class="search__result">Noe results for ' + _this.value + ' found!</div>');
    }).catch(function (err) {
      console.error(err);
    });
  });
  ...
  searchInput.on('keyup', function (e) {
    ...
    if (e.keyCode === 40 && current) {
      next = current.nextElementSibling || items[0]; // will be next OR first one (because if there is no next it will go back to the first)
